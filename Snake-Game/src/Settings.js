class Settings {
    init(params) {
        ...
        if (defaultParams.rowsCount < 10 || defaultParams.rowsCount > 30) {
            throw new Error('Недопустимые настройки: значение rowsCount должно быть в диапазоне 10...30');
        }
        this.rowsCount = defaultParams.rowsCount;

        if (defaultParams.colsCount < 10 || defaultParams.colsCount > 30) {
            throw new Error('Недопустимые настройки: значение colsCount должно быть в диапазоне 10...30');
        }
        this.colsCount = defaultParams.colsCount;

        if (defaultParams.speed < 1 || defaultParams.speed > 10) {
            throw new Error('Недопустимые настройки: значение speed должно быть в диапазоне 1...10');
        }
        this.speed = defaultParams.speed;

        if (defaultParams.winLength < 5 || defaultParams.winLength > 50) {
            throw new Error('Недопустимые настройки: значение winLength должно быть в диапазоне 5...50');
        }
